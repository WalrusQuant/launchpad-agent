...
| `tools` | Tool impls (bash, read, grep, apply_patch, write, webfetch, websearch, skill, plan, todowrite, question, invalid) + orchestrator + registry + `McpToolAdapter`. Subagent-dispatch (`task`) and `lsp` tools are not implemented (no stub code — add when the real feature lands). |
...
- Streaming, tool calls (bash, read, grep, ls, apply_patch, write, glob, webfetch, websearch, skill, update_plan, todowrite, question, invalid)
- **Headless mode** — `lpagent -p "..."` (or `lpagent prompt`) runs one turn and exits with documented codes (0 success / 1 failure / 2 usage). Reads stdin when no prompt is given. Flags: `--model`, `--output-format text|json|stream-json`, `--system-prompt` / `--append-system-prompt`, `--allowed-tools` / `--disallowed-tools`, `--dangerously-skip-permissions`, `-r`/`--resume <id>`, `-c`/`--continue`, `--session-id <uuid>`, `--verbose` / `--debug`. See `crates/cli/src/headless.rs` (`run_headless`) and `crates/cli/src/headless_output.rs` (`--output-format`: `json` emits a Claude-Code-shaped `result` object, `stream-json` emits NDJSON events + a final result; turn failures render in-format with exit 1). Headless drives the **same server runtime as the TUI** via `StdioServerClient`: it spawns a dedicated single-tenant `lpagent server` subprocess, starts a **persisted** session, runs one turn, and prints the final assistant message (captured from `item/completed` events) to stdout. `--system-prompt`/`--append-system-prompt` and `--allowed-tools`/`--disallowed-tools` cross to that subprocess as env vars (`LPA_SYSTEM_PROMPT`, `LPA_APPEND_SYSTEM_PROMPT`, `LPA_ALLOWED_TOOLS`, `LPA_DISALLOWED_TOOLS`) honored at bootstrap (`crates/server/src/bootstrap.rs` + `execution.rs::resolve_turn_model`); `--dangerously-skip-permissions` maps to `session/start` `permission_mode`/`sandbox_mode`. Because every headless run persists a rollout, `--resume <id>` / `--continue` (most-recent in cwd) / `--session-id <uuid>` (resume-or-create) replay them — the dedicated server loads all persisted sessions at bootstrap, so `session/list` + `session/resume` see the full on-disk set. The three resume flags are mutually exclusive (clap `conflicts_with`).
- **Interactive slash commands** — `/help` `/model` `/configure` `/config` `/new` `/rename` `/sessions` `/skills` `/thinking` `/reasoning` `/status` `/compact` `/clear` `/export` `/bug` `/feedback` `/release-notes` `/exit`. `/compact` and `/clear` go through server RPCs (`session/compact`, `session/context/clear`); `#`-prefixed composer input appends a line to the project memory file (`AGENTS.md`/`CLAUDE.md`).
...
- **Subagent dispatch (`task` tool) and `lsp` tool** — not implemented. The old no-op stub code was removed; add real impls + register them in `register_builtin_tools`/`register_builtin_runtime_tools` when the features land.
- **OS-level sandboxing** — no process isolation yet. BUT a rule-based workspace sandbox IS now enforced: set `[sandbox] enabled = true` (optional `workspace_write`) in config.toml and the query loop attaches a `RuleBasedPolicy::with_sandbox` that denies file writes outside cwd and (when `workspace_write = false`) all writes + shell. Sandbox denials override AutoApprove/Ask. See `crates/safety/src/legacy_permissions.rs` (`SandboxContext`, `evaluate_sandbox`) and `crates/core/src/config/safety.rs` (`SandboxConfig`). `SandboxMode::External` is not implemented and now fails closed (denies with "external sandbox mode is not implemented") rather than silently behaving like `Restricted`; no config path currently produces it.
