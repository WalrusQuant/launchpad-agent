...
impl PermissionPolicy for StaticPermissionPolicy {
    async fn decide(
        ...
        // A tool-level approval grants the tool blanket permission regardless of
        // the specific resource. Checked first so caches short-circuit both
        // filesystem and network branches.
        if snapshot
            .approval_cache
            .tool_scopes
        ...
        match request.resource {
            ResourceKind::FileWrite => {
                ...
                // Path-prefix approvals cached from prior decisions short-circuit
                // the Ask branch — this is what makes "approve for the rest of
                // this session" behave like policy allowance.
                if snapshot
                    .approval_cache
                    .path_scopes
                    .iter()
            ...
            ResourceKind::Network => {
                ...
                if snapshot.approval_cache.host_scopes.contains(host) {
                    return Ok(PermissionDecision::Allow);
                }
...
mod tests {
    ...
    use super::{
        ...
        FileSystemPolicyRecord, NetworkPolicy, PermissionDecision, PermissionProfile,
        PermissionRequest, PolicyModelSelection, PolicySnapshot, ResourceKind, SafetyPolicyMode,
        SandboxMode, SandboxPolicyRecord, StaticPermissionPolicy,
    };
    ...
    fn permission_snapshot_with_cache(approval_cache: ApprovalCache) -> PolicySnapshot {
        PolicySnapshot {
            mode: SafetyPolicyMode::StaticPolicy,
            policy_model: PolicyModelSelection::UseTurnModel,
            sandbox_policy: SandboxPolicyRecord {
    ...
    #[tokio::test]
    async fn static_policy_allows_when_tool_scope_cached() {
        let mut cache = ApprovalCache::default();
        cache.tool_scopes.insert("shell_command".into());
        let snapshot = permission_snapshot_with_cache(cache);
        let request = PermissionRequest {
            tool_name: "shell_command".into(),
            resource: ResourceKind::FileWrite,
            action_summary: "write file".into(),
            justification: "tool approved for session".into(),
            path: Some(abs_path("repo/file.rs")),
            host: None,
            target: None,
        };
    ...
    #[tokio::test]
    async fn static_policy_allows_when_path_prefix_cached() {
        let mut cache = ApprovalCache::default();
        cache.path_scopes.insert(abs_path("repo"));
        let snapshot = permission_snapshot_with_cache(cache);
        let request = PermissionRequest {
            tool_name: "write_file".into(),
            resource: ResourceKind::FileWrite,
            action_summary: "write".into(),
            justification: "path prefix approved".into(),
            path: Some(abs_path("repo/subdir/file.rs")),
            host: None,
            target: None,
        };
    ...
    #[tokio::test]
    async fn static_policy_allows_when_host_scope_cached() {
        let mut cache = ApprovalCache::default();
        cache.host_scopes.insert("example.com".into());
        let snapshot = permission_snapshot_with_cache(cache);
        let request = PermissionRequest {
            tool_name: "http_fetch".into(),
            resource: ResourceKind::Network,
            action_summary: "fetch".into(),
            justification: "host approved".into(),
            path: None,
    ...
    #[tokio::test]
    async fn static_policy_still_asks_when_host_scope_missing() {
        let snapshot = permission_snapshot_with_cache(ApprovalCache::default());
        let request = PermissionRequest {
            tool_name: "http_fetch".into(),
            resource: ResourceKind::Network,
            action_summary: "fetch".into(),
            justification: "not approved".into(),
            path: None,
            host: Some("other.com".into()),
            target: None,
        };
