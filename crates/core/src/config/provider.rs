...
use lpa_utils::current_user_config_file;
...
pub enum ProviderWireApi {
    ...
    AnthropicMessages,
    /// Google-compatible Generate Content API.
...
impl ProviderWireApi {
    ...
    pub fn provider_family(self) -> ProviderFamily {
        match self {
            ...
            Self::GoogleGenerateContent => ProviderFamily::google(),
        }
    }
    ...
    pub fn default_for_provider(provider: &ProviderFamily) -> Self {
        match provider {
            ...
            ProviderFamily::Google { .. } => Self::GoogleGenerateContent,
        }
    }
}
...
impl<'de> Deserialize<'de> for ProviderWireApi {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match value.trim().to_ascii_lowercase().as_str() {
            ...
            "google" | "gemini" | "google_generate_content" => Ok(Self::GoogleGenerateContent),
...
pub(crate) fn resolve_provider_settings_from_config(
    ...
    let provider_id = file
        ...
        .context("No provider configured. Run `lpagent onboard` to complete setup.")?;
    let provider_config = file
        .model_providers
        .get(&provider_id)
        .with_context(|| format!("configured provider `{provider_id}` was not found"))?;
    // If the provider has configured models, only honor the top-level
    // `model` when it actually matches one of them — otherwise stale values
    // like a leftover `model = "test-model"` at the top of `config.toml`
    // would silently override a working configured fleet. When the provider
    ...
    let model = file
        .model
        .clone()
        .filter(|_| top_level_model_is_valid)
        ...
        .context("No model configured. Run `lpagent onboard` to complete setup.")?;
