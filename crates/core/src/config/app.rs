...
use lpa_mcp::{McpConfig, McpTransportConfig};
...
pub struct AppConfig {
    ...
    /// MCP server definitions and runtime policy.
    #[serde(default)]
    pub mcp: McpConfig,
...
/// 2. `<workspace>/.lpagent/config.toml` for the currently opened project
...
fn validate_app_config(config: &AppConfig) -> Result<(), AppConfigError> {
    ...
    for server in &config.mcp.servers {
        if !seen_servers.insert(&server.id) {
            return Err(AppConfigError::Validation {
                message: format!("mcp.servers contains duplicate id: {}", server.id),
            });
        }
        match &server.transport {
            McpTransportConfig::Stdio { command, .. } => {
                if command.is_empty() {
                    return Err(AppConfigError::Validation {
                        message: format!(
                            "mcp.servers[{}].transport.command must not be empty",
                            server.id
                        ),
                    });
                }
            }
            McpTransportConfig::StreamableHttp { base_url, .. } => {
                if base_url.trim().is_empty() {
                    return Err(AppConfigError::Validation {
                        message: format!(
                            "mcp.servers[{}].transport.base_url must not be empty",
                            server.id
                        ),
                    });
                }
