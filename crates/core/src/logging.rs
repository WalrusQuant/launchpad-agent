...
mod tests {
    ...
    fn resolve_log_directory_defaults_under_home() {
        let resolved = resolve_log_directory(Path::new("/tmp/.lpagent"), None);
        assert_eq!(resolved, PathBuf::from("/tmp/.lpagent/logs"));
    }
    ...
    fn resolve_log_directory_supports_relative_override_windows() {
        let resolved = resolve_log_directory(
            Path::new("C:\\Users\\tester\\.lpagent"),
            Some(Path::new("diagnostics")),
        );
        assert_eq!(
            resolved,
            PathBuf::from("C:\\Users\\tester\\.lpagent\\diagnostics")
        );
    }
    ...
    fn resolve_log_directory_preserves_absolute_override_windows() {
        let resolved = resolve_log_directory(
            Path::new("C:\\Users\\tester\\.lpagent"),
            Some(Path::new("D:\\lpa-logs")),
        );
        assert_eq!(resolved, PathBuf::from("D:\\lpa-logs"));
    }
    ...
    fn resolve_log_directory_supports_relative_override_unix() {
        let resolved = resolve_log_directory(
            Path::new("/home/tester/.lpagent"),
            Some(Path::new("diagnostics")),
        );
        assert_eq!(resolved, PathBuf::from("/home/tester/.lpagent/diagnostics"));
    }
    ...
    fn resolve_log_directory_preserves_absolute_override_unix() {
        let resolved = resolve_log_directory(
            Path::new("/home/tester/.lpagent"),
            Some(Path::new("/var/log/lpagent")),
        );
        assert_eq!(resolved, PathBuf::from("/var/log/lpagent"));
    }
}
