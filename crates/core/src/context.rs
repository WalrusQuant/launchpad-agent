...
impl Default for TokenBudget {
    fn default() -> Self {
        // `max_output_tokens` needs to leave room for the prompt within the
        // model's context window. Many BYO providers (OpenRouter free routes,
        // local Ollama, Groq) serve models with 8k–32k total context, so a
        // 2048 default fits comfortably without losing agent capability for
        // larger-context builtins. Catalog entries (Claude, GPT, Gemini)
        // override this via `Model.max_tokens` from `models.json`.
        Self::new(200_000, 2048)
    }
}
...
mod tests {
    ...
    fn token_budget_default_values() {
        ...
        // Conservative default so small-context BYO models don't 400 on
        // their own output quota. Catalog entries override via Model.max_tokens.
        assert_eq!(budget.max_output_tokens, 2048);
