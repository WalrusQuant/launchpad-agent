...
fn preset_items(entries: &[crate::app::PresetListEntry]) -> Vec<ListItem<'static>> {
    if entries.is_empty() {
        return vec![ListItem::new(Line::from(vec![Span::styled(
...
fn preset_panel_height(entries: &[crate::app::PresetListEntry]) -> u16 {
    entries
        .len()
        .saturating_mul(2)
        .saturating_add(2)
...
fn inline_preset_panel_height(entries: &[crate::app::PresetListEntry], title: &str) -> u16 {
    entries
        .len()
        .saturating_mul(2)
