...
use lpa_core::{ModelCatalog, PresetModelCatalog};
use textwrap::{Options, wrap};
use unicode_width::UnicodeWidthStr;

use geometry::{
    aux_panel_height, bottom_popup_area, centered_popup_area, composer_popup_area,
    inline_aux_panel_height, onboarding_popup_area, overlay_block,
};
use inline_panel::{inline_aux_panel, render_inline_aux_panel};
use list_items::{model_items, preset_items, session_items, thinking_items};
...
fn composer_title(app: &TuiApp) -> Line<'static> {
    if let Some(prompt) = app.onboarding_prompt.as_deref() {
        return Line::from(vec![
            Span::styled(" configure ", theme::prompt()),
...
fn render_aux_panel_overlay(
    ...
    match &panel.content {
        ...
        AuxPanelContent::PresetList(entries) => {
            ...
            let items = preset_items(entries);
            let mut state = ListState::default();
            if !entries.is_empty() {
                state.select(Some(
                    app.aux_panel_selection.min(entries.len().saturating_sub(1)),
