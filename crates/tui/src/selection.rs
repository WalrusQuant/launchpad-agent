...
use lpa_core::{ModelCatalog, SessionId};
...
impl TuiApp {
    ...
    pub(crate) fn active_preset_id(&self) -> Option<String> {
        ...
        // If we have a saved model entry for the active model, check its base
        ...
        // this is ambiguous — we return "custom" so the summary doesn't lie
        // by claiming "OpenAI" when the user has a non-OpenAI endpoint.
        match self.provider {
            ProviderFamily::Anthropic { .. } => Some("anthropic".to_string()),
            ProviderFamily::Openai { .. } => {
                // Only claim "openai" if no base_url is set (i.e. the default
                // openai.com endpoint). Otherwise we don't know which
                // aggregator is behind it.
    ...
    /// Looks up any previously-saved API key whose base URL matches the
    /// supplied preset URL. Used by `/configure` to pre-populate the api-key
    /// field so users can hit Enter to keep their existing key.
    ...
    pub(crate) fn start_onboarding(&mut self) {
        ...
        // Show a summary of the current config so the user knows what they'd
        // be changing before they commit to a new provider/model.
    ...
    /// Returns a short multi-line summary of the currently active provider,
    /// model, base URL, and masked API key.
    ...
    /// Handles the user's selection in the preset picker.
    ///
    /// Routes into the right follow-up step based on the preset:
    /// - Anthropic/OpenAI/Google: show the existing builtin model catalog
    ///   so users keep the curated experience.
    /// - Any other preset (OpenRouter/Groq/etc): skip straight to "enter
    ///   model slug" using the preset's default base URL.
    /// - Custom: prompt for base URL first since we don't know it.
    pub(crate) fn handle_preset_selected(&mut self, preset_id: &str) {
        ...
        match preset.id {
            "anthropic" | "openai" | "google" => {
                // Keep the builtin model catalog experience for first-party providers.
            ...
            "custom" => {
                // No default URL — fall back to manual entry via the legacy flow.
                self.aux_panel = None;
                self.aux_panel_selection = 0;
                self.onboarding_custom_model_pending = true;
                self.onboarding_selected_model_is_custom = true;
            ...
            _ => {
                // OpenAI-compatible aggregator with a known base URL.
                // Skip URL entry, go straight to API key (or model slug if no key needed).
                self.aux_panel = None;
                self.aux_panel_selection = 0;
                self.onboarding_custom_model_pending = false;
                self.onboarding_selected_model_is_custom = true;
                ...
                if preset.api_key_env_vars.is_empty() {
                    // e.g. Ollama — skip key entry, ask for model directly.
                    self.onboarding_selected_api_key = None;
                    self.onboarding_custom_model_pending = true;
    ...
    pub(crate) fn finish_onboarding_selection(&mut self) -> Result<()> {
        ...
        // Keep in-memory saved_models in sync with config.toml so the new
        // model immediately appears in the `/model` picker without a restart.
...
/// Returns the contents of the active `config.toml` with any `api_key = "..."`
/// lines masked to last-4-chars so the user can safely inspect their config.
fn read_redacted_config_toml() -> Result<String> {
