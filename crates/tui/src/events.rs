...
pub(crate) enum WorkerEvent {
    ...
    /// Server is asking the user to approve a pending tool action.
    ApprovalRequest {
        /// Session that owns the pending approval.
        session_id: lpa_core::SessionId,
        /// Turn that owns the pending approval, when one is active.
        turn_id: Option<lpa_core::TurnId>,
        /// Stable identifier used to correlate with the resolved response.
        approval_id: String,
        /// Concise description of the action awaiting approval.
        action_summary: String,
        /// Rationale shown alongside the prompt.
        justification: String,
    },
    /// A pending approval was resolved (either by this client or another one).
    ApprovalResolved {
        /// The approval request identifier that was resolved.
        approval_id: String,
        /// Short human-readable description of the outcome (e.g. "approved", "denied").
        outcome: String,
    },
}
...
impl TranscriptItem {
    ...
    /// Creates a successful tool-result item rendered as a compact preview.
    ///
    /// Tool outputs land in the preview state (~4 lines with `…` suffix) and
    /// stay there — no pop-in-then-vanish animation. The full body is still
    /// stored on the item so future UX can expand on demand.
    pub(crate) fn live_tool_result(title: impl Into<String>, body: impl Into<String>) -> Self {
        Self::new(TranscriptItemKind::ToolResult, title, body)
            .with_fold_stage(TOOL_RESULT_PREVIEW_STAGE)
    }

    /// Creates a restored historical tool-result item in its compact state.
...
pub(crate) enum TranscriptItemKind {
    ...
    System,
    /// Pending approval prompt awaiting a user decision.
    ApprovalPrompt,
    /// Resolved approval outcome note.
    ApprovalResolution,
}
