...
pub(crate) enum AuxPanelContent {
    ...
    /// Selectable provider-preset list shown on the first step of `/configure`.
    PresetList(Vec<PresetListEntry>),
}

/// One row in the `/configure` provider-preset picker.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct PresetListEntry {
    /// Stable preset id (`anthropic`, `openrouter`, etc).
    pub(crate) id: String,
    /// Human-readable label.
    pub(crate) display_name: String,
    /// Short help line.
    pub(crate) description: String,
    /// Default base URL, if the preset has one.
    pub(crate) default_base_url: Option<String>,
    /// Whether the preset is the currently-active one.
...
pub(crate) struct TuiApp {
    ...
    /// The provider preset the user picked on the first configure step, if any.
    /// `None` before the preset picker; `Some` once a preset has been chosen and
    /// drives the subsequent URL / key / model prompts.
    pub(crate) onboarding_preset_id: Option<String>,
    ...
    /// Whether the transcript should render the full body of reasoning blocks.
    /// When `false` (default), reasoning items collapse to a single line to
    /// keep the viewport stable during streaming; `/reasoning` toggles this.
    pub(crate) show_reasoning: bool,
    /// Whether the current turn has emitted any assistant-facing text yet.
    /// Cleared on user submit, flipped on first assistant delta or completion,
    /// inspected on TurnFinished so the UI can surface an explicit "no
    /// response" marker when the agent ends a turn without saying anything.
    pub(crate) turn_emitted_text: bool,
...
pub(crate) struct PendingApprovalState {
    ...
    pub(crate) session_id: lpa_core::SessionId,
    /// Turn that owns the approval, when one is active on the server.
    pub(crate) turn_id: Option<lpa_core::TurnId>,
    /// Approval identifier used by the server to correlate the response.
