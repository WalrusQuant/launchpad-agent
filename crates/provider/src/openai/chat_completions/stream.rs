...
///
/// When `reqwest_eventsource` returns `InvalidStatusCode`, the server's
/// response body usually contains the actual reason for the rejection
...
/// Reads the response body, truncating to a ~4 KiB preview.
async fn read_body_preview(response: reqwest::Response) -> Option<String> {
    const MAX_PREVIEW_BYTES: usize = 4 * 1024;
    let bytes = response.bytes().await.ok()?;
    let slice = if bytes.len() > MAX_PREVIEW_BYTES {
    ...
    let stream = async_stream::try_stream! {
        ...
        while let Some(event) = event_source.next().await {
            let event = match event {
                Ok(event) => event,
                Err(error) => {
                    Err::<(), _>(build_stream_error(&request.model, error).await)?;
                    unreachable!("try_stream! error branch always returns early");
                }
            };
...
impl ChatCompletionStreamState {
    ...
    fn apply_tool_call_delta(
        ...
        if !entry.started && tool_call_delta_starts_block(entry) {
            entry.started = true;
            // Some OpenAI-compatible providers stream a blank tool-call id;
            // synthesize a stable one so the result routes back to this call.
            // Persist it on the entry so later deltas and the tool result
