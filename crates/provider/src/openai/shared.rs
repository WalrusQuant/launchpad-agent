use lpa_protocol::{RequestRole, ToolDefinition};
...
/// Some OpenAI-compatible providers (notably Z.ai's coding plan / GLM) omit
/// `tool_calls[].id` even though the spec requires it. A blank id collides
/// across parallel tool calls, so tool results route to the wrong call or get
/// dropped. Synthesize a unique id when the upstream value is empty.
