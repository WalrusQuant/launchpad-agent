...
async fn stdio_server_process_supports_handshake_and_session_start() -> Result<()> {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_lpa-server"))
        .env("LPA_HOME", home_dir.path().join(".lpagent"))
        .env("USERPROFILE", home_dir.path())
        .env("HOME", home_dir.path())
        .env("LPA_PROVIDER", "openai")
        .stdin(std::process::Stdio::piped())
        .stdout(std::process::Stdio::piped())
        .stderr(std::process::Stdio::piped())
        .spawn()
        .context("spawn lpa-server child process")?;
