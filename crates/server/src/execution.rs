...
pub struct ServerRuntimeDependencies {
    ...
    /// MCP manager owning all configured MCP server supervisors.
    pub(crate) mcp_manager: Arc<dyn McpManager>,
    /// Snapshot of exposed tool names belonging to servers configured with
...
impl ServerRuntimeDependencies {
    ...
    pub fn mcp_manager(&self) -> Arc<dyn McpManager> {
        Arc::clone(&self.mcp_manager)
    }

    /// Builds a fresh `ApprovalCache` pre-seeded with trusted MCP tool names.
    pub(crate) fn new_approval_cache(&self) -> ApprovalCache {
        let mut cache = ApprovalCache::default();
        for name in self.trusted_mcp_tool_names.iter() {
            cache.tool_scopes.insert(name.clone());
        }
        cache
    }
    ...
    ///
    /// Precedence:
    /// 1. Requested slug matches a builtin catalog entry → use it as-is.
    /// 2. Requested slug is set but unknown to the catalog → treat it as a BYO
    ///    model (e.g. OpenRouter/Groq/Ollama slugs) and synthesize a default
...
pub(crate) struct RuntimeSession {
    ...
    /// Approval scopes remembered for the session so the orchestrator can
    /// short-circuit repeat `Ask` prompts without re-bothering the user. Also
    /// consulted by `StaticPermissionPolicy::decide` when policy-driven
    /// decisions migrate onto the new trait. Shared with `ToolContext` during
    /// turn execution so both the server-side responder and the tool
    /// orchestrator see the same state.
    pub(crate) approval_cache: Arc<Mutex<ApprovalCache>>,
}
