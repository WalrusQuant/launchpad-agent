...
impl RolloutStore {
    ...
    // All 8 fields are load-bearing parts of the persisted session identity;
    // grouping into a builder would trade one lint for extra boilerplate.
    ...
    /// Persists a compaction snapshot: writes the canonical JSON snapshot file
    /// under `<data_root>/snapshots/<session_id>/<turn_id>.json` and appends a
    /// `RolloutLine::CompactionSnapshot` pointer to the rollout journal. Both
    /// writes are required for durability; if the JSON write fails the
    /// rollout-line append is skipped so a resumed session cannot reference a
    ...
    pub(crate) fn append_compaction_snapshot(
        ...
        let snapshot_store = SnapshotStore::new(self.data_root.join("snapshots"));
...
impl ReplayState {
    ...
    fn into_runtime_session(self, deps: &ServerRuntimeDependencies) -> Result<RuntimeSession> {
        ...
        Ok(RuntimeSession {
            ...
            approval_cache: std::sync::Arc::new(tokio::sync::Mutex::new(deps.new_approval_cache())),
        })
    }
