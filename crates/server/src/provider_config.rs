...
pub fn load_server_provider(
    ...
    let env_provider = env_non_empty("LPA_PROVIDER");
    let env_wire_api = env_non_empty("LPA_WIRE_API");
    let env_model = env_non_empty("LPA_MODEL");
    let env_base_url = env_non_empty("LPA_BASE_URL");
    let env_api_key = env_non_empty("LPA_API_KEY");
    ...
    let api_key = env_api_key
        ...
        .or_else(|| env_non_empty("GOOGLE_API_KEY"))
    ...
    let provider: std::sync::Arc<dyn ModelProviderSDK> = match wire_api {
        ...
        ProviderWireApi::GoogleGenerateContent => {
            ...
            std::sync::Arc::new(GoogleProvider::new(base_url).with_api_key(api_key))
        }
    };
...
fn default_model_for_provider(provider: ProviderFamily) -> String {
    match provider {
        ...
        ProviderFamily::Google { .. } => "gemini-2.5-flash".to_string(),
    }
}
...
fn parse_wire_api(value: &str) -> Option<ProviderWireApi> {
    match value.trim().to_ascii_lowercase().as_str() {
        ...
        "google" | "gemini" | "google_generate_content" => {
            Some(ProviderWireApi::GoogleGenerateContent)
        }
        _ => None,
    }
}
