...
pub struct FileSystemConfigPathResolver {
    /// The resolved user-level config directory (for example `~/.launchpad/agent`).
...
impl FileSystemConfigPathResolver {
    ...
    pub fn from_env() -> Result<Self, ConfigPathError> {
        let user_config_dir =
            find_lpa_home().map_err(|_| ConfigPathError::HomeDirectoryUnavailable)?;
...
mod tests {
    ...
    fn resolver_builds_user_and_project_paths() {
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/repo/.lpagent"))
        );
        assert_eq!(
            paths.project_config_file,
            Some(PathBuf::from("/repo/.lpagent/config.toml"))
        );
    }
    ...
    fn resolver_supports_user_only_paths_windows() {
        let resolver =
            FileSystemConfigPathResolver::new(PathBuf::from("C:\\Users\\tester\\.lpagent"));
        ...
        assert_eq!(
            paths.user_config_file,
            PathBuf::from("C:\\Users\\tester\\.lpagent\\config.toml")
        );
    }
    ...
    fn resolver_supports_user_only_paths_unix() {
        let resolver = FileSystemConfigPathResolver::new(PathBuf::from("/home/tester/.lpagent"));
        let paths = resolver.resolve_paths(None).expect("paths");
        ...
        assert_eq!(
            paths.user_config_file,
            PathBuf::from("/home/tester/.lpagent/config.toml")
        );
    }
}
