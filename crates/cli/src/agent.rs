...
use lpa_tui::{InteractiveTuiConfig, SavedModelEntry, TerminalMode, run_interactive_tui};
...
pub async fn run_agent(
    ...
    let lpa_core::ResolvedProviderSettings {
        provider,
        model,
        base_url: _,
        api_key: _,
    ...
    let resolved = if onboarding_mode {
        ...
        ResolvedProviderSettings {
            ...
            wire_api: match &fallback_model.provider {
                ...
                ProviderFamily::Google { .. } => ProviderWireApi::GoogleGenerateContent,
            },
...
fn server_env_overrides(resolved: &lpa_core::ResolvedProviderSettings) -> Vec<(String, String)> {
    let mut env = vec![
        (
...
mod tests {
    ...
    use lpa_core::{Model, PresetModelCatalog, ProviderConfigFile};
    use lpa_protocol::ProviderFamily;
