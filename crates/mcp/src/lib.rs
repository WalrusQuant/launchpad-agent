...
pub use manager::{StdMcpManager, snapshot_status};
...
pub struct McpServerRecord {
    ...
    pub enabled: bool,
    /// The trust level for tools exposed by this server. `Trusted` skips the
    /// approval flow; `Prompt` (default) routes through it.
    #[serde(default)]
    pub trust_level: TrustLevel,
}

/// Controls whether a configured MCP server's tools bypass the approval flow.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum TrustLevel {
    /// Tools from this server route through the approval flow like any other
    /// non-read-only tool. This is the safe default.
    #[default]
    Prompt,
    /// Tools from this server skip the approval flow entirely. Appropriate for
    /// local/self-hosted servers the user has explicitly vetted.
    Trusted,
}
...
pub struct McpToolDescriptor {
    ...
    /// Optional safety / semantic annotations from MCP ≥ 2025-03-26.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub annotations: Option<protocol::ToolAnnotations>,
}
