...
//! - raw preset/config concerns live in `lpa-core`
...
/// Google-family API surfaces supported by the runtime.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum GoogleApi {
    GenerateContent,
...
impl ProviderFamily {
    ...
    /// Creates the default Google provider selection.
    pub fn google() -> Self {
        Self::Google {
            api: GoogleApi::default(),
        }
    }
    ...
    D: serde::Deserializer<'de>,
{
    ...
    match value {
        serde_json::Value::String(text) => match text.trim().to_ascii_lowercase().as_str() {
            ...
            "google" | "gemini" => Ok(ProviderFamily::google()),
