# Launchpad Agent Detailed Specification: Tools

## Background and Goals

`lpagent` is a tool-using coding agent, so tools are not an implementation detail. They are a first-class runtime subsystem that sits between model output, safety policy, execution backends, and conversation persistence.
...
- support built-in and runtime-native tools owned by `lpa-tools`
...
- MCP server discovery and MCP tool/resource bridging before those capabilities are handed to `lpa-tools`
...
- built-in tools are owned and implemented directly in `lpa-tools`
- MCP-contributed capabilities are specified in [spec-mcp.md](./spec-mcp.md) and only enter this layer after normalization
- skills are not tools and must not be modeled as tool definitions in `lpa-tools`
...
- `lpa-tools` remains the crate for tool contracts and built-in tool implementations
