...
`lpa-core` owns the normalized runtime application configuration. This config
...
2. user-level config at `LPA_HOME/config.toml`
3. project-level config at `<workspace>/.lpagent/config.toml`
...
`LPA_HOME` and reads the user and project TOML files. It can also carry CLI
...
- user config: `LPA_HOME/config.toml`
- project config: `<workspace>/.lpagent/config.toml`
