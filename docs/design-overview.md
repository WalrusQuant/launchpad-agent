# Launchpad Agent Design
This document defines the specification for an SWE coding agent named lpagent, it is Claude Code / Codex Inspired. The documentation's goal is to guide implementation.
...
- `lpa-core`: conversation, model integration, context management, coding workflow orchestration, long-running task execution, and shared runtime state
- `lpa-tools`: built-in tool contracts and tool execution adapters
- `lpa-safety`: sandboxing, redaction, approvals, and safety policy
- `lpa-mcp`: MCP server lifecycle, discovery, and capability bridging
- `lpa-server`: transport-neutral runtime API server
- `lpa-utils`: shared low-level utilities that are broadly reusable and not owned by a higher-level domain crate
...
- Session: A conversation between a user and the lpagent agent. Each session contains multiple turns. Each session is identified by a UUID v7, which provides time-ordered uniqueness and enables efficient sorting and lookup.
...
## 1.6 MCP
MCP support allows `lpagent` to connect to external capability providers that expose tools, resources, and resource templates through the Model Context Protocol.
