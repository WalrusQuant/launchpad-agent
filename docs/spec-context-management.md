# Launchpad Agent Detailed Specification: Context Management
...
`lpa-core::code` or a future `lpagent-git` integration layer owns:
...
- Whether git-backed snapshot capture should stay in `lpa-core::code` or move into a dedicated git integration module if the feature grows.
